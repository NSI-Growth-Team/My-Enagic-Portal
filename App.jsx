...
const GOOGLE_APPS_SCRIPT_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxVQiZWIBb8TB_BOiwpqNRtZU-pSSzRQqLqXKm2IxuxmAesPRYyXSc6Er_GkiRpSk7H8A/exec'; // <--- THIS IS THE LINE YOU NEED TO EDIT
...
const App = () => {
    ...
    const [isAdmin, setIsAdmin] = useState(false); // This will be determined by the Apps Script
    const [loading, setLoading] = useState(true);
    const [currentPage, setCurrentPage] = useState('dashboard'); // Default page

    useEffect(() => {
        // Simulate user authentication based on the provided token or anonymous sign-in
        // In a real app, you'd use a proper auth service (e.g., Firebase Auth, Google OAuth)
        // For this example, we'll just use a fixed user ID and email
        const simulateAuth = async () => {
            let currentUserId = 'anonymous_user'; // Default anonymous
            let currentUserEmail = 'anonymous@example.com'; // Default anonymous

            if (initialAuthToken) {
                // In a real scenario, you'd decode the token or use a Firebase Auth listener
                // For this example, we'll just use a fixed user ID and email
                currentUserEmail = `user_${Math.random().toString(36).substring(2, 11)}@example.com`; // Unique email for testing
                currentUserId = `user_${Math.random().toString(36).substring(2, 11)}`;
            } else {
                // If no token, use a truly anonymous identifier
                currentUserEmail = `anonymous_${Math.random().toString(36).substring(2, 11)}@example.com`;
            ...
            try {
                // Ensure the URL is not the placeholder before making the fetch call
                if (GOOGLE_APPS_SCRIPT_WEB_APP_URL === 'YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE') {
                    throw new Error("Google Apps Script URL is not configured. Please replace 'YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE' with your deployed Apps Script URL.");
                }
...
const Navbar = ({ currentPage, setCurrentPage }) => {
    const { userId, userEmail, isAdmin } = useContext(AppContext);
...
const UserDetailForm = () => {
    const { userEmail, GOOGLE_APPS_SCRIPT_WEB_APP_URL } = useContext(AppContext);
...
const JourneyTrackerForm = () => {
    ...
    const journeySteps = [
        ...
        { label: "MachineDelivered", name: "MachineDelivered" },
    ];
...
// Dashboard component
const Dashboard = () => {
    const { userEmail, isAdmin, GOOGLE_APPS_SCRIPT_WEB_APP_URL } = useContext(AppContext);
    ...
    const [dailyActivityNotification, setDailyActivityNotification] = useState(''); // New state for notification

    // Filter states
    const [startDate, setStartDate] = useState('');
    const [endDate, setEndDate] = useState('');
    const [selectedTaskType, setSelectedTaskType] = useState('All'); // 'All', 'Morning', 'Evening'
